...
void DrawIndexedInstancedExecutorDX11<TVertex,TInstance>::Execute( PipelineManagerDX11* pPipeline, IParameterManager* pParamManager )
{
	...
	if ( InstanceBuffer.GetElementCount() > 0 ) {
        ...
        DrawExecutorDX11<TVertex>::VertexBuffer.UploadData( pPipeline );
        DrawIndexedExecutorDX11<TVertex>::IndexBuffer.UploadData( pPipeline );
		...
		int layout = DrawExecutorDX11<TVertex>::GetInputLayout( pPipeline->ShaderStages[VERTEX_SHADER]->DesiredState.ShaderProgram.GetState() );
		pPipeline->InputAssemblerStage.DesiredState.InputLayout.SetState( layout );
		pPipeline->InputAssemblerStage.DesiredState.PrimitiveTopology.SetState(DrawExecutorDX11<TVertex>::m_ePrimType );

		pPipeline->InputAssemblerStage.DesiredState.VertexBuffers.SetState( 0, DrawExecutorDX11<TVertex>::VertexBuffer.GetBuffer()->m_iResource );
		...
		pPipeline->InputAssemblerStage.DesiredState.IndexBuffer.SetState(DrawIndexedExecutorDX11<TVertex>::IndexBuffer.GetBuffer()->m_iResource );
		...
		// instanced draw call.
		if ( m_uiCount == 0 ) {
			pPipeline->DrawIndexedInstanced(DrawIndexedExecutorDX11<TVertex>::IndexBuffer.GetElementCount(), InstanceBuffer.GetElementCount(), 0, 0, 0 );
		} else {
			pPipeline->DrawIndexedInstanced(DrawIndexedExecutorDX11<TVertex>::IndexBuffer.GetElementCount(), m_uiCount, 0, 0, m_uiStart );
		}
	}
}
...
void DrawIndexedInstancedExecutorDX11<TVertex,TInstance>::ResetGeometry()
{
	...
	ResetInstances();
	DrawIndexedExecutorDX11<TVertex>::ResetGeometry();
}
