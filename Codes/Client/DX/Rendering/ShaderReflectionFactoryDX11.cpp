	...
	for ( UINT i = 0; i < desc.ConstantBuffers; i++ )
	{
		...
		if ( bufferDesc.Type == D3D_CT_CBUFFER || bufferDesc.Type == D3D_CT_TBUFFER )
		{
			...
			BufferLayout.pParamRef = pParamMgr->GetConstantBufferParameterRef( 
				UtilString::ToUnicode( std::string( BufferLayout.Description.Name ) ) );
			...
			for ( UINT j = 0; j < BufferLayout.Description.Variables; j++ )
			{
				...
				{
					pParam = pParamMgr->GetVectorParameterRef( UtilString::ToUnicode( std::string( var_desc.Name ) ) );
				}
							...
							( type_desc.Class == D3D_SVC_MATRIX_COLUMNS ) )
				{
					...
					if ( count == 0 ) 
					{
						pParam = pParamMgr->GetMatrixParameterRef( UtilString::ToUnicode( std::string( var_desc.Name ) ) );
					}
					else
					{
						pParam = pParamMgr->GetMatrixArrayParameterRef( UtilString::ToUnicode( std::string( var_desc.Name ) ), count );
					}
				}
