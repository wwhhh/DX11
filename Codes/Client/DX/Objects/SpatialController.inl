...
void SpatialController<T>::Update( float fTime )
{
	...
	if (IController<T>::m_pEntity )
	{
		IController<T>::m_pEntity->Transform.Rotation().Rotation( m_vRotation );
		IController<T>::m_pEntity->Transform.Position() = m_vTranslation;
	}
...
void SpatialController<T>::MoveForward( const float distance )
{
	if ( IController<T>::m_pEntity )
	{
		m_vTranslation += IController<T>::m_pEntity->Transform.Rotation().GetRow( 2 ) * distance;
	}
}
...
void SpatialController<T>::MoveRight( const float distance )
{
	if ( IController<T>::m_pEntity )
	{
		m_vTranslation += IController<T>::m_pEntity->Transform.Rotation().GetRow( 0 ) * distance;
	}
}
...
void SpatialController<T>::MoveUp( const float distance )
{
	if ( IController<T>::m_pEntity )
	{
		m_vTranslation += IController<T>::m_pEntity->Transform.Rotation().GetRow( 1 ) * distance;
	}
}
